...
export async function runResearchAgent(
	...
	if (!apiKey)
		return {
			runId,
			proposalCount: 0,
			errors: [`No API key configured for ${provider}. Go to AI Settings.`],
		}
	...
	onProgress?.({
		phase: 'research',
		pct: 20,
		message: `Researching ${person.givenName} ${person.surname}...`,
	})
	...
	for (const pp of parsed) {
		...
		if (validation.valid) {
			...
			errors.push(
				`Rejected: ${pp.fieldName} = "${pp.newValue}" — ${validation.violations.join('; ')}`
			)
		}
	}
...
export async function runBatchResearch(
	...
	onProgress?.({
		phase: 'done',
		pct: 100,
		message: `Done: ${totalProposals} proposals from ${personXrefs.length} people`,
	})
...
function buildPersonContext(person: Person, events: any[], parents: any, _families: any[]): string {
