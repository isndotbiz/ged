...
export async function findSources(
	...
	onProgress?.({
		phase: 'init',
		pct: 5,
		message: `Searching for ${person.givenName} ${person.surname}...`,
	})
	...
	onProgress?.({
		phase: 'familysearch',
		pct: 20,
		message: 'Searching FamilySearch (22.7B records)...',
	})
	...
	onProgress?.({
		phase: 'proposals',
		pct: 70,
		message: `Processing ${sourcesFound} records found...`,
	})
	...
	for (const result of allResults) {
		...
		const proposal: Omit<Proposal, 'id'> = {
			...
			reasoning: `Found in ${result.collection || 'public records'}. ${Object.entries(
	...
	onProgress?.({
		phase: 'done',
		pct: 100,
		message: `Found ${sourcesFound} records, created ${proposalsCreated} proposals`,
	})
