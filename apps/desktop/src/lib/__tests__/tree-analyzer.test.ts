...
describe('tree-analyzer', () => {
	...
	it('flags tree-only and no-source quality warnings and skips both for validated people', async () => {
		...
		const issues = await analyzeTree()
		expect(
			issues.some((i) => i.personXref === 'I1' && i.title === 'Online tree sources only')
		).toBe(true)
		expect(issues.some((i) => i.personXref === 'I2' && i.title === 'No source citations')).toBe(
			true
		)
		expect(
			issues.some(
				(i) =>
					i.personXref === 'I3' &&
					(i.title === 'Online tree sources only' || i.title === 'No source citations')
			)
		).toBe(false)
	})
})
