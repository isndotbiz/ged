...
describe('relationship-finder', () => {
	...
	it('returns null for unconnected persons', async () => {
		...
		vi.mocked(getPerson).mockImplementation(
			async (xref: string) => ({ xref, givenName: xref, surname: '' }) as any
		)
	...
	it('returns a clear same-person path and summary', async () => {
		...
		expect(path).toEqual([{ personXref: 'I1', personName: 'Ada Lovelace', relationship: 'self' }])
	...
	it('terminates on circular family references without hanging', async () => {
		...
		const result = await Promise.race([
			...
			new Promise<null>((_, reject) =>
				setTimeout(() => reject(new Error('relationship search timeout')), 200)
			),
		])
