...
export async function exportSubsetGedcom(
	xrefs: string[],
	format: '5.5.1' | '7.0' = '5.5.1'
): Promise<string> {
	if (xrefs.length === 0) return ''
	const db = await getDb()
	const placeholders = xrefs.map((_, i) => `$${i + 1}`).join(', ')
	const persons = await db.select<Person[]>(
		`SELECT * FROM person WHERE xref IN (${placeholders}) ORDER BY xref`,
		xrefs
	)
	...
	const media = await db.select<GedcomMedia[]>(
		`SELECT * FROM media WHERE ownerXref IN (${placeholders})`,
		xrefs
	)
	...
	for (const f of families) {
		...
		const children = await db.select<{ childXref: string }[]>(
			`SELECT childXref FROM child_link WHERE familyXref = $1`,
			[f.xref]
		)
		for (const child of children) {
...
function _formatGedDate(d: Date): string {
	const months = [
		'JAN',
		'FEB',
		'MAR',
