...
export async function scanAndMatchMedia(
  ...
  const db = await getDb();
  const report: MatchReport = { total: 0, matched: 0, unmatched: 0, created: 0, linked: 0, results: [] };
...
function matchFileToPersons(
  filePath: string,
  bySurname: Map<string, Person[]>
): MatchResult {
  ...
  if (candidates.length === 0) {
    ...
    for (const part of filenameParts) {
      ...
      if (bySurname.has(normalized)) {
        ...
        if (bestMatch) return { filePath, matchedPerson: bestMatch, matchType: 'fuzzy', confidence: 0.6 };
...
export async function deduplicateMedia(
  ...
  for (let i = 0; i < dupes.length; i++) {
    ...
    const rows = await db.select<{id: number; xref: string}[]>(
      `SELECT id, xref FROM media WHERE filePath = $1 ORDER BY id`, [group.filePath]
    );
    ...
    for (const rm of removeRows) {
      ...
      const links = await db.select<{personXref: string; role: string; isPrimary: number}[]>(
        `SELECT personXref, role, isPrimary FROM media_person_link WHERE mediaId = $1`, [rm.id]
      );
...
function scoreDuplicate(p1: Person, p2: Person): { score: number; reasons: string[] } {
  ...
  if (g1 && g2 && g1 === g2) { score += 0.3; reasons.push('Same given name'); }
  else if (g1 && g2 && (g1.startsWith(g2) || g2.startsWith(g1))) { score += 0.15; reasons.push('Similar given name'); }

  // Same birth date
  if (p1.birthDate && p2.birthDate) {
    if (p1.birthDate === p2.birthDate) { score += 0.3; reasons.push('Same birth date'); }
    else {
      const y1 = p1.birthDate.match(/(\d{4})/)?.[1];
      const y2 = p2.birthDate.match(/(\d{4})/)?.[1];
      if (y1 && y2 && y1 === y2) { score += 0.1; reasons.push('Same birth year'); }
    }
  }

  // Same birth place
  if (p1.birthPlace && p2.birthPlace && p1.birthPlace.toLowerCase() === p2.birthPlace.toLowerCase()) {
    score += 0.2; reasons.push('Same birth place');
  }

  // Same death date
  if (p1.deathDate && p2.deathDate && p1.deathDate === p2.deathDate) {
    score += 0.2; reasons.push('Same death date');
  }
...
export async function mergePersons(keepXref: string, removeXref: string): Promise<{ success: boolean; error?: string }> {
  const db = await getDb();
  try {
    // Snapshot the removed person for undo
    const removedPerson = await db.select<Person[]>(`SELECT * FROM person WHERE xref = $1`, [removeXref]);
    const removedEvents = await db.select<any[]>(`SELECT * FROM event WHERE ownerXref = $1`, [removeXref]);
    const removedMediaLinks = await db.select<any[]>(`SELECT * FROM media_person_link WHERE personXref = $1`, [removeXref]);
    ...
    await db.execute(`DELETE FROM media_person_link WHERE personXref = $1 AND id NOT IN (
      SELECT MIN(id) FROM media_person_link WHERE personXref = $1 GROUP BY mediaId
    )`, [keepXref]);
    // Transfer child links
    await db.execute(`UPDATE OR IGNORE child_link SET childXref = $1 WHERE childXref = $2`, [keepXref, removeXref]);
    // Transfer family partner references
    await db.execute(`UPDATE family SET partner1Xref = $1 WHERE partner1Xref = $2`, [keepXref, removeXref]);
    await db.execute(`UPDATE family SET partner2Xref = $1 WHERE partner2Xref = $2`, [keepXref, removeXref]);
    ...
    if (keep.length > 0 && removedPerson.length > 0) {
      const k = keep[0]; const r = removedPerson[0];
      const updates: string[] = [];
      const vals: any[] = [];
      let i = 1;
      if (!k.birthDate && r.birthDate) { updates.push(`birthDate = $${i++}`); vals.push(r.birthDate); }
      if (!k.birthPlace && r.birthPlace) { updates.push(`birthPlace = $${i++}`); vals.push(r.birthPlace); }
      if (!k.deathDate && r.deathDate) { updates.push(`deathDate = $${i++}`); vals.push(r.deathDate); }
      if (!k.deathPlace && r.deathPlace) { updates.push(`deathPlace = $${i++}`); vals.push(r.deathPlace); }
      if (updates.length > 0) {
        vals.push(keepXref);
...
export async function unmergePersons(mergeLogId: number): Promise<{ success: boolean; error?: string }> {
  const db = await getDb();
  try {
    ...
    await db.execute(
      ...
      [removedPerson.xref, removedPerson.givenName, removedPerson.surname, removedPerson.suffix, removedPerson.sex, removedPerson.isLiving, removedPerson.birthDate, removedPerson.birthPlace, removedPerson.deathDate, removedPerson.deathPlace, removedPerson.sourceCount, removedPerson.mediaCount, removedPerson.personColor || '', removedPerson.proofStatus || 'UNKNOWN']
    );
    ...
    for (const ml of removedMediaLinks) {
      await db.execute(
        ...
        [ml.mediaId, ml.personXref, ml.role, ml.isPrimary, ml.sortOrder, ml.faceX, ml.faceY, ml.faceW, ml.faceH, ml.caption, ml.addedBy, ml.verified, ml.createdAt]
      );
    }
