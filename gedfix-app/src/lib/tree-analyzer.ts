...
export async function analyzeTree(onProgress?: (pct: number, msg: string) => void): Promise<TreeIssue[]> {
  ...
  const allChildLinks = await db.select<{familyXref: string; childXref: string}[]>('SELECT * FROM child_link');
  ...
  for (const p of persons) {
    ...
    if (birthYear && deathYear && deathYear < birthYear) {
      addIssue('date', 'critical', p.xref, '', 'Death before birth',
        `${p.givenName} ${p.surname}: born ${p.birthDate}, died ${p.deathDate}`,
        'Verify and correct birth/death dates', true, 'Swap birth and death dates');
    }

    // 2. Age over 120
    if (birthYear && deathYear && (deathYear - birthYear) > 120) {
      addIssue('date', 'warning', p.xref, '', 'Unreasonable age (>120)',
    ...
    if (birthYear && birthYear > currentYear) {
      addIssue('date', 'critical', p.xref, '', 'Future birth date',
        `${p.givenName} ${p.surname}: born ${p.birthDate}`,
        'Correct birth date', true, 'Clear birth date');
    }

    // 4. No name
    if (!p.givenName && !p.surname) {
      addIssue('quality', 'warning', p.xref, '', 'Missing name',
    ...
    if (events.length === 0) {
      addIssue('quality', 'info', p.xref, '', 'No events recorded',
    ...
    if (birthYear && birthYear < 1000) {
      addIssue('date', 'warning', p.xref, '', 'Very early birth date',
        `${p.givenName} ${p.surname}: born ${p.birthDate}`,
  ...
  for (const fam of families) {
    ...
    if (marrYear) {
      if (p1) {
        ...
        if (birthYear && (marrYear - birthYear) < 14) {
          addIssue('date', 'warning', p1.xref, fam.xref, 'Marriage before age 14',
      ...
      if (p2) {
        ...
        if (birthYear && (marrYear - birthYear) < 14) {
          addIssue('date', 'warning', p2.xref, fam.xref, 'Marriage before age 14',
    ...
    for (const child of children) {
      ...
      for (const parent of [p1, p2]) {
        ...
        if (parentBirth && (childBirth - parentBirth) < 12) {
          addIssue('relationship', 'warning', child.xref, fam.xref, 'Parent too young',
      ...
      for (const parent of [p1, p2]) {
        ...
        if (parentDeath && childBirth > parentDeath + 1) {
          addIssue('relationship', 'critical', child.xref, fam.xref, 'Born after parent death',
    ...
    if (fam.partner1Xref && fam.partner1Xref === fam.partner2Xref) {
      addIssue('relationship', 'critical', fam.partner1Xref, fam.xref, 'Person married to self',
        `Family ${fam.xref} has same person as both partners`,
        'Correct family record', true, 'Remove duplicate partner from family');
    }

    // 12. No partners in family
    if (!fam.partner1Xref && !fam.partner2Xref) {
      addIssue('relationship', 'warning', '', fam.xref, 'Family with no partners',
        `Family ${fam.xref} has no partner records`,
        'Add partner information or remove family', true, 'Delete empty family record');
    }
  }
  ...
  for (const [key, group] of nameMap) {
    if (group.length > 1) {
      ...
      for (const [year, dupes] of birthYears) {
        if (dupes.length > 1) {
          addIssue('duplicate', 'warning', dupes[0].xref, '',
